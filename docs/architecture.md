...
- **BYOK policy and metadata.** Oxy owns provider-connection status, scope,
  validation and customer authorization. Kaana owns encrypted secret custody
  under an opaque handle and can execute only the exact credential generation
  signed onto an authorized route. It never selects a connection, grants BYOK
  from provider/account metadata or falls back to a platform credential; the
  customer lane also neither reads nor updates the platform credential lane's
  deployment breaker. The custody, resolver and production gates are specified in
  `customer-provider-credentials.md`.
...
1. **Answered by contract 1.3.0: the envelope carries an ordered authorized
   ...
   provenance.
2. **Answered by contract 2.0.0: a `routing_profile_id` target carries the exact
   opaque Oxy profile ID and is executed only through its `authorizedRoutes`.**
   Kaana never resolves that ID by name, slug, position or database order. The
   entries name every executable destination; a profile without them is refused
   with `param: authorizedRoutes`. Request schema v1 remains transitional only
   for direct model targets, and its old slug target is refused.
...
13. **A model-scope `route_switch` cannot be constructed for a pinned concrete
    ...
    model line. An exact routing-profile-ID target gets that line from its first
    authorized route and can therefore report an explicitly authorized
    cross-model switch; a concrete request that pinned a revision cannot and
    must never substitute those weights.
