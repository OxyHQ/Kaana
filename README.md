...
Kaana is the inference **data plane** for the Oxy platform. It normalizes a
...
([ADR 0005][adr0005], [ADR 0006][adr0006]); Kaana executes what Oxy has already
...
`Relay` was the working name, and the git history also passes through `Pensara`
for one commit — a name that was chosen and replaced before anything shipped
under it, so no deployment, variable or header ever carried it. Kaana is the
name; the product's domain is `kaana.ai`.

Everything a person reads now says Kaana: the module path, the packages, the
binaries' source directories, the environment variables and the docs.
...
- **Environment variables.** Every variable is `KAANA_*`; a deployment still
  setting `RELAY_*` is answered from that instead, current name winning. One
  function, `providerconfig.EnvName`.
- **The Oxy edge's signing headers.** `X-Oxy-Kaana-Key-Id`, `-Timestamp` and
...
- The image's own `ENV` defaults. A container definition's `environment` beats
  an image `ENV`, but the binary prefers `KAANA_*` — so an image setting the
...
```
cmd/kaana/                      the serving binary: env config, wiring, graceful drain
cmd/kaana-publisher/            the publishing binary: builds the inventory and re-issues it
...
internal/providercost/          what a request cost Kaana upstream; never a customer amount
internal/publisher/             builds the inventory from the providers' own model lists
internal/kaana/                 the executor: routing, failover, framing, usage reports
...
nothing of Kaana's, which is what lets the drift gate compare it against the
