...
/* -------------------------------------------------------------------------- */
/*  What Kaana implements, and what it deliberately does not                  */
/* -------------------------------------------------------------------------- */

// goShapes maps a published shape to the Go type that carries it on the wire.
//
// Every shape Kaana reads from or writes to the wire is here. A shape that is
...
// with the members Kaana declares. The members are compared as an exact,
...
var notApplicable = map[string]string{
	...
	"publisherSlugSchema":                   "catalogue: a component of a model id Kaana never splits",
	"modelSlugSchema":                       "catalogue: a component of a model id Kaana never splits",
	...
	// Money and the ledger are Oxy's. Kaana measures units and never prices
	// them; there is deliberately no Money type in this package.
	"currencyCodeSchema":            "ledger: Kaana never quotes an amount",
	"exactDecimalSchema":            "ledger: Kaana never quotes an amount",
	"moneySchema":                   "ledger: Kaana never quotes an amount",
	...
	"usageReceiptSchema":            "ledger: settlement is Oxy's; Kaana emits normalizedUsageReport",
	...
	// only routingPolicyReferenceSchema, which Kaana does implement.
	...
	"externalPaymentSchema":         "billing: payment processing is Oxy's, and Kaana holds no payment credential",
...
// TestPublishedGrammarsMatchGoPatterns compares the regexes Kaana actually
// enforces against the published ones, character for character.
//
// Only the patterns Kaana validates against are checked. The rest of the
...
func TestPublishedGrammarsMatchGoPatterns(t *testing.T) {
	...
	for name, pattern := range map[string]string{
		...
		if published == "" {
			t.Errorf("%s publishes no regex; Kaana enforces one", name)
			continue
		}
...
func (c *shapeChecker) compareField(shape, jsonName string, node descriptorNode, field goField) []string {
	...
	case !optional && field.omitEmpty:
		problems = append(problems, fmt.Sprintf("%s: the contract requires it; the Go tag has omitempty, so Kaana can omit a required field", where))
...
func (c *shapeChecker) compareRef(where, ref string, goType reflect.Type) []string {
	...
	if reason, excused := notApplicable[ref]; excused {
		return []string{fmt.Sprintf("%s: references %s, which is recorded not-applicable (%s) yet appears in a shape Kaana exchanges", where, ref, reason)}
	}
...
// contract and not in Go is a value Kaana would not handle, and something in Go
// and not in the contract is a value Kaana could emit that Oxy rejects.
