...
// Relay's own transport to the Oxy edge, so it is deliberately minimal — one
...
func (d *Decoder) Next() (Event, bool) {
	...
	for d.scanner.Scan() {
		...
		default:
			// `id:`, `retry:` and unknown fields carry nothing Relay reads.
		}
	}
...
//
// The name is Relay's own transport framing, not part of the published
