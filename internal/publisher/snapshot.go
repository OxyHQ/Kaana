...
//
// The reader's struct is what Pensara ACCEPTS; this is what the publisher
...
// makes the operator's `PENSARA_PROVIDERS` the primary-choosing knob, in one
...
func BuildSnapshot(discoveries []Discovery, attribution *Attribution, previous Observations, at time.Time) (BuildResult, error) {
	if len(discoveries) == 0 {
		return BuildResult{}, fmt.Errorf("publisher: no provider reported any models, so a snapshot would declare nothing and Pensara would refuse it")
	}
	...
	// SAME code Pensara runs, with the same horizon, before they leave this
	// process. A snapshot that would fail Pensara's parse is a snapshot that
	...
	if _, err := inventory.Parse(body, inventory.DefaultMaxSnapshotAge); err != nil {
		return BuildResult{}, fmt.Errorf("publisher: the built snapshot is one Pensara would refuse: %w", err)
	}
...
func snapshotComment() []string {
	return []string{
		"GENERATED by cmd/pensara-publisher. Do not edit: the next re-issue overwrites it.",
		...
		"ORDER IS LOAD-BEARING. Failover is off by default, so a reference resolves to the deployment declared FIRST. Deployments appear in the order PENSARA_PROVIDERS declares their providers, and only providers holding a credential are declared at all.",
		"IT HOLDS NOTHING OXY OWNS: no account, application, credential, price or commercial permission. Provider credentials resolve from the environment in cmd/pensara and are never here.",
		"STALENESS IS MEASURED FROM `issuedAt`. This file is re-issued on a cadence shorter than PENSARA_INVENTORY_MAX_AGE even when nothing has changed, because an unchanged snapshot with an old issuedAt is indistinguishable from a publisher that has stopped.",
	}
}
