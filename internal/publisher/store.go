...
import (
	...
	"github.com/OxyHQ/Relay/internal/awssig"
...
func NewS3Store(client *http.Client, bucket, key, region string, credentials CredentialSource) (*S3Store, error) {
	...
	case bucket == "":
		...
		return nil, fmt.Errorf("publisher: no bucket: set RELAY_INVENTORY_BUCKET; there is no default, because publishing to a guessed bucket succeeds silently")
	case key == "":
		return nil, fmt.Errorf("publisher: no object key: set RELAY_INVENTORY_KEY")
	case region == "":
