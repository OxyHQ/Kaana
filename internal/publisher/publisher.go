// Package publisher builds Relay's deployment inventory from what the
...
// with an old `issuedAt` is indistinguishable, from Relay, from a control plane
...
// At fifteen minutes, four consecutive publishes can fail before Relay stops
...
type Config struct {
	...
	// RELAY_PROVIDERS that serves it without either refusing its references or
